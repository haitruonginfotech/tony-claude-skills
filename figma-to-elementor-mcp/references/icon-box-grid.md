# 🔥 STRICT Icon Box Grid Processing Skill

If the target section is an **Icon Box Grid**, you MUST ABANDON default generative logic and strictly conform to the exact JSON mapping below. Deviating from these rules will break the environment.

## 1. 🏛️ Detection Criteria
A section qualifies as "Icon Box Grid" when ALL of the following are true:
- A **centered heading** sits at the top of the section
- Below the heading is a **grid of cards**, each card containing:
  - A **small SVG icon** at the top — typically 30–60px, transparent/monochrome background, NOT a photo or illustration
  - A **text description** below the icon
- Cards may have a consistent border, shadow, and background color

> **Quick visual test:** If the card graphic is small and sits on a transparent or flat-color background → Icon Box Grid. If it fills a large portion of the card like a photo → use Image Box Grid instead.
...
You MUST view and strictly copy the node structures from `resources/elementor-icon-box-grid-304-2026-05-06.json` if you are ever unsure of the exact syntax.
